...
export class NamespaceRegistry {
   ...
   * @returns 规范化后的命名空间名称
   */
  private normalizeNamespace(scriptId: string, namespace: string, isShared: boolean): string {
    // 共享命名空间不添加前缀
    if (isShared) {
      return namespace
    }
   ...
   * @returns 原始命名空间名称
   */
  private denormalizeNamespace(scriptId: string, normalizedNamespace: string): string {
  ...
  registerScript(scriptId: string, namespaceConfig: ScriptNamespaceConfig): void {
    ...
    // 规范化并存储配置
    const normalizedConfig: ScriptNamespaceConfig = {}
    for (const [namespace, config] of Object.entries(namespaceConfig)) {
